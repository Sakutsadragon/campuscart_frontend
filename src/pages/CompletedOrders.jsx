...
import { faArrowLeft, faCheckCircle, faHome, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
function Completedorders() {
    ...
    const fetchCompletedOrders = async (page) => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(getcompletedordersRoute(seller.username, page, 10));
            if (res.data.orders.length > 0) {
                ...
                setHasMoreOrders(false); // No more orders to fetch
            }
        } catch (err) {
        ...
        <Container>
            ...
            <Section>
                {completedOrders.length > 0 ? (
                    <OrdersGrid>
                        {completedOrders.map(order => (
                            <OrderCard key={order._id}>
                                ...
                                <OrderStatus>
                                    <FontAwesomeIcon icon={faCheckCircle} style={{ color: "#2ecc71", marginRight: "5px" }} />
                                    Delivered
                                </OrderStatus>
                            </OrderCard>
                        ))}
                    ...
                    <NoOrdersMessage>
                        <FontAwesomeIcon icon={faCheckCircle} style={{ fontSize: "1.5rem", color: "#95a5a6", marginRight: "10px" }} />
