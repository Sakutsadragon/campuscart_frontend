...
import { sregisterRoute } from '../utils/APIRoutes';
function SRegister() {
    ...
    useEffect(()=>{
        if(localStorage.getItem("seller-user"))
            navigate('/shome')
    },[])
    ...
    const clisubmission= async(event)=>{
        ...
        if(validentry()){
            ...
            const {data}= await axios.post(sregisterRoute,{
                username,
                email,
                password,
            });
            ...
            if(data.status === true){
                localStorage.setItem("seller-user",JSON.stringify(data.user))
                navigate("/shome");
            }
